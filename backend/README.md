...
## What Is Left To Implement (Backend)

1. Add automated tests:
- Unit tests for controllers and utilities
- Integration tests for tenant isolation and role access
- Contract/API tests for all route groups

2. Expand transactional consistency:
- Apply Mongo transactions to other multi-write flows (homework create/update, payment status transitions, report creation where applicable)
- Ensure activity log and outbox writes stay in the same transaction in those flows

3. Replace outbox processor mock send with real delivery:
- Integrate provider (SMTP/SendGrid/etc.)
- Add dead-letter handling and delivery metrics

4. Add authorization hardening and validation:
- Centralized request schema validation (Joi/Zod/express-validator)
- Stronger per-endpoint payload constraints and sanitization

5. Add operational readiness:
- Structured logging and request correlation IDs
- Rate limiting and security headers
- Production CORS policy and environment-specific config

6. Improve report lifecycle integration:
- Add Cloudinary delete/sync hooks when monthly report metadata is deleted
- Optional checksum validation pipeline for uploaded PDFs

7. Add archival and lifecycle jobs:
- Activity log archival policy (12+ months)
- Outbox failed-item cleanup monitoring and alerting
