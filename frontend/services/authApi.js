...
const PROFILE_NOT_FOUND_CODE = 'profile/not-found';
  ...
  try {
    ...
    if (!existingProfile) {
      throw createServiceError({
        ...
        fallbackMessage: 'Account profile is missing. Please sign in again or contact support.',
      });
    }
    ...
    if (isTransientProfileError(error) || error?.code === 'operation/timeout') {
      throw createServiceError({
        ...
        fallbackMessage: 'Could not load your account role right now. Please try again.',
      });
    }
...
const syncUserProfileStrict = async ({ uid, email, name, role, isCreate = false }) => {
  await withOperationTimeout(
    withRetry(
      () =>
        upsertUserProfile({
