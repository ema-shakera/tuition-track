...
export default function HomeScreen({ navigation }) {
  ...
  const hasAnyDashboardData = Boolean(stats.length || homeworkItems.length || tuitionItems.length);
  const showRefreshLoading = isLoading && hasAnyDashboardData;
  const feedSections = useMemo(
    () => (hasAnyDashboardData ? ['stats', 'homework', 'tuition', 'actions'] : ['empty', 'actions']),
    [hasAnyDashboardData]
  );
        ...
        return (
          <View style={styles.section}>
            <EmptyState
              ...
              description={
                isTeacherView
                  ? 'Create your first tuition or homework record to populate this dashboard.'
                  : 'Your dashboard will show classes, tuition, and homework when your teacher assigns them.'
              }
            />
          </View>
        );
          ...
          <View style={styles.headerContent}>
            ...
            {!error && isFallbackData ? (
              <StatusBanner
                type="info"
                title="Sync notice"
                message="Some dashboard sections are still syncing. Pull to refresh if recent changes are missing."
